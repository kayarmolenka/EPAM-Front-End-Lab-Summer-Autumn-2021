...
function add(...arr) {
    return arr.reduce((acc, obj) => {
        Object.entries(obj).forEach(([key, value]) => (!acc[key]) ? acc[key] = value : acc[key] = acc[key] + value)
    return acc;        
    }, {})
}

console.log('Add :', add(a1, b1))

const a = {z: 4, t: 6};
const b = {x: 1, y: 2, z: 4, t: 6, r: 4};
const c = {x: 1, y: 2, z: 4, t: 6};
const d = {x: 1, y: 2, t: 6, a: 4};
const e = {t: 6, x: 1, z: 4};

function intersection(...arr) {
    const mainObject = arr.reduce((acc, item) => ({...acc, ...item}));
    const keys = Object.keys(mainObject)
    
    return keys.reduce((acc, key) => {
        const isIntersect = arr.every(item => item[key]);
        if(isIntersect) {
            return {
                ...acc,
                [key]: mainObject[key]
